...
public final class ExpressionTypeParser {
    ...
    public static Either<ExpressionType, ValidationResult.Builder.ElementStep> parse(final CharSequence charSequence) {
        try {
            return Eithers.makeLeft(charSequence != null ? PARSER.parse(charSequence) : ExpressionTypes.TOP());
        } catch (final ParserException e) {
            return Eithers.makeRight(ValidationResult.init.message("Could not parse FEEL expression type '" + charSequence + "'"));
        }
    }
}
